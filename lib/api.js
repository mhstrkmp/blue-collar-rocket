...
async function authenticateUser(collection, username, passwordInput) {
  try {
    const { token, password } = await dbCollection(collection).findOne({
      name: username,
    });
    if ((await bcrypt.compare(passwordInput, password)) === false) {
      return {};
    }
    if ((await bcrypt.compare(passwordInput, password)) === true) {
      return token;
    }
  } catch (error) {
    console.error(error);
